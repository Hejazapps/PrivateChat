...
open class GrowingTextView: UITextView {
    ...
    var delegate1: GrowingTextViewDelegate?
    ...
    private func associateConstraints() {
        ...
        for constraint in constraints {
            if constraint.firstAttribute == .height && constraint.relation == .equal {
               ...
               // print("i got height text \(heightConstraint?.constant ?? 0)")
            }
        }
    }
    ...
    override open func layoutSubviews() {
        ...
        if height != heightConstraint!.constant {
           // print("is needed \(height)")
            shouldScrollAfterHeightChanged = true
            heightConstraint!.constant = height
            delegate1?.textViewDidChangeHeight?(self, height: height)
